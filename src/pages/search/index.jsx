...
import { Filter, ChevronDown, SlidersHorizontal, Loader2, SearchX } from 'lucide-react';
...
const CATEGORIES = ['Indoor', 'Outdoor', 'Succulents', 'Flowering', 'Low Maintenance'];
const TRENDING_KEYWORDS = ['Areca Palm', 'Snake Plant', 'Peace Lily', 'Office', 'Gift'];
...
function SearchPage() {
  ...
  const isValidSearch = searchTerm && searchTerm.trim().length >= 2;
  ...
  const handlePriceCommit = () => {
    setCurrentPage(1);
    updateUrlParams({ minPrice: priceRange[0], maxPrice: priceRange[1], page: 1 });
  };
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => searchPlants({ search: searchTerm, options: apiOptions }),
  ...
  }, [initialCategory, initialSort, initialMinPrice, initialMaxPrice, initialPage]);

  // Card Props Helpers
  const getImagesForCard = (images = []) => {
    if (!images || images.length === 0) return ['https://placehold.co/400x600?text=No+Image'];
    return images.sort((a, b) => (b.isPrimary ? 1 : -1)).map(img => img.mediaUrl);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-white">
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8"> {/* Increased max-width for 5 columns */}
        
        {/* Breadcrumbs */}
        <nav className="mb-6 text-sm text-gray-500 hidden sm:block">
          <span onClick={() => navigate("/")} className='hover:underline hover:cursor-pointer'>Home</span> &gt;
        ...
        {!searchTerm && (
           <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
             <h2 className="text-2xl font-bold text-gray-900 mb-4">Trending Searches</h2>
...
*/}
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">
                  {searchTerm ? `Results for "${searchTerm}"` : 'All Plants'}
                </h2>
                <p className="text-gray-600 text-sm">
                  {!isLoading && !isError && `${totalItems} plant${totalItems !== 1 ? 's' : ''} found`}
                </p>
              </div>
            ...
            {isLoading ? (
               <div className="flex flex-col items-center justify-center h-64">
                ...
                <div className="text-center py-16 text-red-500 bg-red-50 rounded-xl border border-red-100">
                   ...
                   <p className="text-sm mt-1">{error?.message || "Please try refreshing the page."}</p>
                </div>
                ...
                >
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">No plants found for "{searchTerm}"</h3>
                  <button onClick={() => navigate('/category/plants')} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors shadow-sm">View All Plants</button>
                </motion.div>
            ) : (
              // --- UPDATED GRID LAYOUT ---
              <motion.div 
                ...
                {productsData.map((product) => (
                    <ProductCard
                      ...
                      price={{
                        ...
                        discount: product.discountPercent > 0 ? `${Math.round(product.discountPercent)}% OFF` : null
                      }}
                      images={getImagesForCard(product.images)}
                      tags={getProductTags(product.systemAttributes)}
                      colors={product.color ? [product.color.name] : []}
                      hexColors={product.color ? [product.color.hexCode] : []}
                      rating={product.rating}
                      reviews={product.totalRatings}
                      onAddToCart={() => console.log('Add to cart', product.variantId)}
                    />
                ))}
              </motion.div>
            )}
            ...
            {/* Pagination */}
            {totalPages > 1 && !isLoading && !isError && productsData.length > 0 && (
              <div className="flex flex-wrap justify-center gap-2 mt-8">
                <button onClick={() => handlePageChange(Math.max(currentPage - 1, 1))} disabled={currentPage === 1} className="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-green-200 rounded-md hover:bg-green-50 disabled:opacity-50">Previous</button>
                <span className="flex items-center px-4 text-sm font-medium text-gray-700">Page {currentPage} of {totalPages}</span>
                <button onClick={() => handlePageChange(Math.min(currentPage + 1, totalPages))} disabled={currentPage === totalPages} className="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-green-200 rounded-md hover:bg-green-50 disabled:opacity-50">Next</button>
              </div>
            )}
          </div>
        </div>
